...
def plot_missing_values(df, title, color):
    ...
    missing_df = pd.DataFrame(
        {'column': missing_ratio.index, 'missing_ratio': missing_ratio.values})

    plt.figure(figsize=(15, 6))
    plt.grid(True)
    ax = sns.barplot(x='column', y='missing_ratio',
                     data=missing_df, color=color)
...
def compare_test_train_categorical(train, test, cat_cols=[]):
    for col in cat_cols:
